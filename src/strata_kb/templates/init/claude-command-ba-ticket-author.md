...
when empty and a `missions/` directory is present, run `kb mission next`
first and propose the first `ready` story. A `blocked` story may be
drafted only with its reasons acknowledged by the BA and carried into
the handover; otherwise ask for it during Intake.
